//
//  AllClubsViewController.swift
//  Clubhub
//
//  Created by Lebbeus Woods on 4/18/18.
...
class AllClubsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    override func viewDidLoad() {
        ...
        if let userid = defaultValues.string(forKey: "userid") {
            ...
            Alamofire.request(URL_ALL_CLUBS, method: .get,  parameters: parameters).responseJSON { response in
                //printing response
                //print(response)
        ...
        if cell.mapLbl.text! == cell.mapLbl.text
        {
            ...
            Alamofire.request(URL_THIS_CLUB, method: .post,  parameters: parameters).responseJSON { response in
                ...
                if let result = response.result.value {
                    ...
                    let JoinClubViewController = self.storyboard?.instantiateViewController(withIdentifier: "JoinClubViewController") as! JoinClubViewController
                    self.navigationController?.pushViewController(JoinClubViewController, animated: true)
